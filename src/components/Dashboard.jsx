    ...
    Promise.allSettled([
      ...
      .then(([userRes, pickupRes]) => {
        ...
        if (pickupRes.status === "fulfilled") {
          const pickupList = pickupRes.value?.data || pickupRes.value || [];
          if (Array.isArray(pickupList) && pickupList.length > 0) {
            const completedCount = pickupList.filter(
              (p) => (p.status || "").toLowerCase() === "completed"
            ).length;
            const pendingCount = pickupList.filter(
              (p) => (p.status || "").toLowerCase() === "pending"
            ).length;
            const pts = completedCount * 25 + pickupList.length * 10;
            ...
            setActivities(
              pickupList.slice(0, 5).map((p) => ({
                id: p.id,
                status: p.status === "in_progress" ? "In Progress" : p.status.charAt(0).toUpperCase() + p.status.slice(1),
                text: `Request #${p.id} ${p.status.replace("_", " ")} (${p.waste_type}, ${p.quantity} ${p.quantity_unit || "kg"})`,
              }))
            );
          }
        }
    ...
    <div className="db-container">
      <header className="db-header">
        ...
        <nav className="db-nav">
          ...
          <Link to="/pickup-requests" className="nav-btn" style={{ textDecoration: "none", display: "inline-flex", alignItems: "center" }}>
            Pickup Requests
          </Link>
          <Link to="/volunteer" className="nav-btn" style={{ textDecoration: "none", display: "inline-flex", alignItems: "center" }}>
            Volunteer Hub
          </Link>
          <button
            className="nav-btn"
